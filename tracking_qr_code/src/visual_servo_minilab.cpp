  ...
  try {
    ...
    if (! valid_pose || Z <= 0) {
      ...
      if ( i == 0) {
 	  ...
 	  if (j == 0) {
        	out_cmd_vel.linear.x = 0.2;
        	out_cmd_vel.linear.y = 0;
        	out_cmd_vel.linear.z = 0;
        	out_cmd_vel.angular.x = 0;
        	out_cmd_vel.angular.y = 0;
        	out_cmd_vel.angular.z = -0.2;
        	pubTwist_.publish(out_cmd_vel);
    ...
    if (  v[0] < 0.1 && v[0] > 0.001 ) {
      	out_cmd_vel.linear.x = abs(v[0]*10); }
    else if ( v[0] < 0 && v[0] > -0.07) {
    ...
    if ( v[1] < 0.1  )  {
      	out_cmd_vel.angular.z = v[1]*10; }
    else if ( v[1] < -0.003) {
  	out_cmd_vel.linear.x = 0 ;}

    else  { out_cmd_vel.angular.z = v[1]; }
